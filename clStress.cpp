...
#define KERNEL_FILE "clStress.cl"


int
ClStress::setupConstantBandwidth()
{
...
int
ClStress::genBinaryImage()
{
    bifData binaryData;
    binaryData.kernelName = std::string(KERNEL_FILE);
...
ClStress::setupCL(void)
{
    ...
    buildData.kernelName = std::string(KERNEL_FILE);
...
int
ClStress::bandwidth(cl_kernel &kernel)
{
    cl_int status;
